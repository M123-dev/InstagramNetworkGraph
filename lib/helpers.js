...
let getAllFollowingPath = (username) => {
	if (username === constants.username_to_scrape && !constants.you_follow) {
		return `${appRoot}/data/allFollowers/allFollowers-${username}.json`;
	}

	return `${appRoot}/data/allFollowing/allFollowing-${username}.json`;
};


// Store profile
let storePersistentUser = (username, user_obj) => {
...
let get_user_info = async (username, page, wait_time) => {
	...
	try {
		...
		console.log(
			`Error happened while parsing profile, please check https://i.instagram.com/api/v1/users/web_profile_info/?username=${username} to work with headless=false`
		);
	}
	storePersistentUser(username, cut_extended_user);
...
let get_user_all_following = async (
	...
	let url = `https://i.instagram.com/api/v1/friendships/${id}/`;

	if (username === constants.username_to_scrape && !constants.you_follow) {
		url = url + "followers/?search_surface=follow_list_page&";
	} else {
		url = url + "following/?";
	}

	url = url + `count=${following_count}`;


	let next_max_id;
	let should_continue = true;
	let all_following_list = [];
	...
	while (should_continue) {
		...
		if(next_max_id === undefined) {
			...
			await page.goto(url + `&max_id=${next_max_id}`);
		}
		...
		try {
			...
			console.log(
				"Error happened while parsing root profile, please check https://i.instagram.com/api/v1/users/web_profile_info/?username=<yourUsername> to work with constants.headless=false"
			);
		}

		all_following_list = [
			...all_following_list,
			...all_following_obj["users"],
		];

		if(all_following_obj["next_max_id"] === undefined || all_following_obj["next_max_id"] >= following_count){
			should_continue = false;
		} else {
			next_max_id = all_following_obj["next_max_id"];
			console.log(`${username} has a lot of connections, needs multiple queries`);
		}
	...
	fs.writeFile(
		...
		JSON.stringify({users:all_following_list}),
		function (err) {
			if (err) {
				console.log(
