...
let get_user_all_following = async (id, username, page, following_count, wait_time) => {
  ...
  try {
    ...
    console.log("Error happend while parsing root profile, please check https://i.instagram.com/api/v1/users/web_profile_info/?username=<yourUsername> to work with constants.headless=false")
  }
  ...
  fs.writeFile(
    ...
    function (err) {
      if (err) {
        console.log(`The all following file from ${username} could not be written.`, err);
      }
    }
  );
