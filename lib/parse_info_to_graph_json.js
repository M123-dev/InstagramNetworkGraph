...
function calculate_mutual_amount(source, target) {
  ...
  if (
    ...
    return constants.normal_link_size;
  }
...
function normalize_node_size() {
  ...
  let average = parseInt((following_amount.reduce((a,b) => a + b) / following_amount.length));  
  for (let i = 0; i < nodes.length; i++) {
    let node = nodes[i];
    let target_following_amount = get_profile_info_cache(node.id)["edge_follow"]["count"];
    let new_size = parseInt(node.size / (target_following_amount + average) * constants.multiply_node); 
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if ((node.id === constants.username_to_scrape)) {
      nodes[i].size = Math.min(max, constants.max_node);
      continue;
    }
