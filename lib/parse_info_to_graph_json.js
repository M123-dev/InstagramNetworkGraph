...
function push_link(a, b) {
  ...
  if (!constants.use_edge_mutual_followed_by) {
    length = calculate_link_length(a, b);
  }
...
function handle_people_root_is_following(x_profile_info) {
  ...
  if (constants.use_edge_mutual_followed_by) {
    size = calculate_link_length(constants.username_to_scrape, username);
  }
...
function calculate_link_length(source, target) {


   // Don't calculate distance between other persons, works by removing the following tree lines.
   if(source !== constants.username_to_scrape && target !== constants.username_to_scrape){
    return 300;
  }
...
// The initial link size is set to the amount of mutual friends, 
...
function invert_link_length() {
  ...
  for(let i = 0; i < links.length; i++){
    links[i].length = ((-1 * links[i].mutual_amount + max) + constants.min_link_size);
  }

  // Not needed but good for debugging data.json
  links = links.sort(function(a, b){return b - a});
}
