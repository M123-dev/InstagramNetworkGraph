...
async function parse_info_to_graph_json() {
	...
	config_obj = {
		...
		colorful: constants.colorful,
		max_node: constants.max_node,
	};
...
function push_link(a, b, length_param, isFollowedByRootUser) {
	if (
		links.some(
			(e) =>
	...
	if (length_param !== undefined) {
		length = length_param;
	} else if(!isFollowedByRootUser){
		length = constants.normal_link_size;
...
function handle_all_following(username, root_all_following_list) {
	...
	const all_following_list = get_all_following_cache(username) || [];

	let length = all_following_list.length;

	for (let i = 0; i < length; i++) {
		let nested_username = all_following_list[i]["username"];
		let nested_profile_pic = all_following_list[i]["profile_pic_url"];
		...
		if (!isFollowedByRootUser && !constants.show_people_without_direct_connection) {
			continue;
		}

		push_link(username, nested_username, undefined, isFollowedByRootUser);

		if (!nodes.some((el) => el.id === nested_username)) {
			nodes.push({
				id: nested_username,
				profile_pic_url: nested_profile_pic,
				isFollowedByRootUser: false,
				size:1,
			});
		} else {
			const index = nodes.indexOf(nodes.find(element => element.id === nested_username));
...
function normalize_node_size() {
	...
	for (let i = 0; i < nodes.length; i++) {
		...
		if(node.isFollowedByRootUser === false){
			continue;
		}
