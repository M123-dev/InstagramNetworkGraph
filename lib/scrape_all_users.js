...
async function scrape_all_users() {
  ...
  const all_following_obj = JSON.parse(fs.readFileSync(helper.getAllFollowingPath(constants.username_to_scrape)));
  ...
  const division_constant = constants.use_edge_mutual_followed_by ? 1 : 2;
  const ms_per_houer = 3600000;
  let wait_time = (ms_per_houer / constants.requests_per_hour) / division_constant;
  ...
  for (let i = 0; i < length; i++) {
    ...
    if(user_info_obj["is_private"] == true && user_info_obj["followed_by_viewer"] == false){
      console.log(`(${i}/${length - 1}) User: ${user_info_obj["username"]} is private skipping it's all following file, consider using [lib/remove_private_accounts.js] to remove all private accounts, then you can rescrape them with your main account. \n(${i}/${length - 1}) ALERT: Removing without rescraping breaks the diagramm`);
      continue;
    }
