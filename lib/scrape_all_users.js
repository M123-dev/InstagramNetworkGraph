...
async function scrape_all_users() {
	...
	let wait_time = ms_per_hour / constants.requests_per_hour;
	...
	for (let i = 0; i < length; i++) {
		...
		if (fs.existsSync(helper.getDataPath(username))) {
			...
			// If we already scraped the user we load it's persistent data to be able to query all_following
      ...
      user_info_obj["followed_by_viewer"] == false
			...
			console.log(
				...
				} is private skipping it's all following file, consider using [lib/remove_private_accounts.js] to remove all private accounts, then you can re-scrape them with your main account. \n(${i}/${
					length - 1
				}) ALERT: Removing without re-scraping breaks the diagram`
			);

			fs.writeFile(
				helper.getAllFollowingPath(username),
				JSON.stringify({
					users: [],
				}),
				function (err) {
